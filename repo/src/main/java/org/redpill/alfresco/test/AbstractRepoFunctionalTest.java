...
public abstract class AbstractRepoFunctionalTest {
  ...
  protected JSONObject getMetadata(String nodeRef) throws JSONException {
    Response response = given().contentType(JSON).baseUri(getBaseUri()).pathParam("nodeRef", nodeRef).expect().statusCode(200).when().get("/api/metadata?nodeRef={nodeRef}&shortQNames=true");
  ...
  protected JSONObject checkoutDocument(String nodeRef) throws JSONException {
    ...
    Response response = given().contentType(JSON).baseUri(getBaseUri()).pathParam("store_type", storeType).pathParam("store_id", storeId).pathParam("id", id).body("{}").expect().statusCode(200).when()
